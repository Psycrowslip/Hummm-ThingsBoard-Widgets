self.onInit = async function () {
  ...
  const currentCustomerId = currentUser.customerId;
  ...
  const assetUrl =
    currentUser.authority === "TENANT_ADMIN"
      ...
      : `/api/customer/${currentCustomerId}/assets?pageSize=1000&page=0&includeCustomers=true`;

  /* ============================== SAFE ROOT ============================== */
  const ROOT = window.top || window.parent || window;
  if (!ROOT._TB_SHARED) ROOT._TB_SHARED = {};
  const SHARED = ROOT._TB_SHARED;
  const GLOBAL_HUB_EVENT = "dh16:hubUpdated";
  ...
  const CONFIG = {
    ...
    REFRESH_MS: 90 * 1000,       // ⚡ 90s — segment logic doesn't need 30s cadence
    MIN_SEG_MS: 50 * 1000,
    CHUNK_MS: 6 * 60 * 60 * 1000,

    /* ── Concurrency / resilience tunables ── */
    ASSET_CONCURRENCY: 4,    // ⚡ bumped from 2 — safe with batched keys + semaphore
    DEVICE_CONCURRENCY: 5,   // ⚡ bumped from 3 — each device is now 1 batched call
    GLOBAL_MAX_INFLIGHT: 10, // ⚡ raised from 8 — fewer total requests thanks to batching
    HTTP_TIMEOUT_MS: 45000,  // per-request timeout
    RETRY_COUNT: 2,          // retries on failure (total attempts = 1 + RETRY_COUNT)
    RETRY_BASE_MS: 1500,     // base delay before first retry
  };

  const TELEMETRY_KEYS = ["num_of_targets", "State", "zone", "ED_tier"];
  const TELEMETRY_KEYS_CSV = TELEMETRY_KEYS.join(",");
  ...
  const ATTRIBUTE_KEYS_CSV = ATTRIBUTE_KEYS.join(",");
  ...
  const TZ_CACHE = {};
  const ASSET_META_CACHE = {};
  const DEVICE_META_CACHE = {};

  /* =========================================================================
     GLOBAL SEMAPHORE — caps total in-flight telemetry requests
     Prevents flooding the ThingsBoard server regardless of how many
     assets/devices/chunks we're processing in parallel.
  ============================================================================ */
  ...
  async function httpGetWithRetry(url, opts = {}) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      if (isDestroyed) throw new Error("Widget destroyed");

      // Back off before retries (not before first attempt)
      if (attempt > 0) {
        const delay = CONFIG.RETRY_BASE_MS * Math.pow(2, attempt - 1);
        const jitter = Math.random() * delay * 0.3;
        await new Promise((r) => setTimeout(r, delay + jitter));
        console.warn(`[DataHub] Retry ${attempt}/${maxRetries} for ${url.slice(0, 80)}…`);
      }
      ...
      try {
        ...
        lastErr = err;
        // Don't retry on 4xx client errors (only on timeouts / 5xx)
  ...
  /* =========================================================================
     ...
     Each individual HTTP call acquires the global semaphore so we never
     exceed GLOBAL_MAX_INFLIGHT simultaneous requests to the server.
  ============================================================================ */
  ...
  /* ─────────────────────────────────────────────────────────────────────────
     BATCH ALL KEYS + SEQUENTIAL CHUNKS (with global concurrency gate)
     Chunks run sequentially per device to avoid request storms, but the
     global semaphore allows multiple devices to interleave their chunks.
     On failure of a batched call, falls back to per-key fetches.
  ───────────────────────────────────────────────────────────────────────────── */
  async function fetchAllKeysBatched(devId, keys, queryStartTs, endTs) {
    ...
    // ⚡ Run chunks in PARALLEL — the global semaphore caps total in-flight
    const chunkResults = await Promise.all(
      chunkBounds.map(async ({ start, end }) => {
        try {
          return { ok: true, data: await fetchTelemetryChunkGated(devId, keysCsv, start, end), start, end };
        } catch (batchErr) {
          console.warn(`[DataHub] Batch failed for ${devId} chunk ${start}-${end}, trying per-key`, batchErr);
          // Fallback: fetch each key individually for this chunk
          const fallback = {};
          for (const key of keys) {
            try {
              ...
              console.warn(`[DataHub] Key ${key} also failed for ${devId}, skipping`, keyErr);
              fallback[key] = [];
            }
          }
          return { ok: true, data: fallback, start, end };
        }
      }),
    );

    // Merge in time order (chunkBounds are already sorted)
  ...
  const HUB_EVENT = `dh16:hubUpdated:${WIDGET_UID}`;
...
<div class="dh16-root" id="dh16-root">
  ...
  async function loadTelemetry({ fullReload = false } = {}) {
    ...
    if (fullReload) {
      clearSeriesCursor();
      // ⚡ Invalidate meta caches so stale names/labels are re-fetched
    ...
    const stageAssets = {};
    const prevAssetObj = fullReload ? {} : SHARED.DATAHUB_V16?.sharedAssetObj || {};
    ...
    await mapLimit(assetIds, CONFIG.ASSET_CONCURRENCY, async (assetId) => {
      ...
      await mapLimit(deviceIds, CONFIG.DEVICE_CONCURRENCY, async (devId) => {
        const attrUrl =
          `/api/plugins/telemetry/DEVICE/${devId}` +
          `/values/attributes/SERVER_SCOPE?keys=${ATTRIBUTE_KEYS_CSV}`;
        ...
        try {
          [devMeta, rawAttributes, batchedTelemetry] = await Promise.all([
            ...
            httpGetWithRetry(attrUrl, { retries: 1 }),
            fetchAllKeysBatched(devId, TELEMETRY_KEYS, batchQueryStart, endTs),
          ]);
        } catch (err) {
          console.error(`[DataHub] Device ${devId} failed entirely, skipping`, err);
          failedDevices++;
          done++;
          updateProgress();
          return; // skip this device, don't block the rest
        }
    ...
    const segmentTz = Object.values(stageAssets)[0]?.timezone || "Etc/UTC";
    const stageSegments = { house: [], timezone: segmentTz };
    if (hasTargets) stageSegments.house = buildHouseSegmentsFromStageAssets(stageAssets);

    SHARED.DATAHUB_V16 = freezeHubPayload({
    ...
    nextRefreshAt = SHARED.DATAHUB_V16.hubMeta.refreshAt;
  ...
  modalCard.style.cssText = `
    width:980px;max-width:calc(100vw - 40px);max-height:86vh;
    background:#f3f5ef;border-radius:14px;overflow:hidden;
    box-shadow:0 10px 32px rgba(0,0,0,0.28);display:flex;flex-direction:column;
  `;
  modal.appendChild(modalCard);
  document.body.appendChild(modal);

  function getAssetsList() {
    const obj = SHARED.DATAHUB_V16?.sharedAssetObj || {};
    return Object.entries(obj).map(([assetId, a]) => ({
      assetId, assetName: a.assetName || assetId,
    }));
  }
  function getDeviceList(assetId) {
    const devs = SHARED.DATAHUB_V16?.sharedAssetObj?.[assetId]?.devices || {};
    return Object.entries(devs).map(([deviceId, d]) => ({
      deviceId, label: d.label || deviceId, dataKeys: d.dataKeys || [],
    }));
  }
  function getKeySeries(assetId, deviceId, keyName) {
    const dev = SHARED.DATAHUB_V16?.sharedAssetObj?.[assetId]?.devices?.[deviceId];
    const dk = dev?.dataKeys?.find((k) => k.name === keyName);
    return dk?.data || [];
  }
  function fmtTs(ts, tzIana) {
    if (!ts) return "—";
    const opts = tzIana ? { timeZone: tzIana } : undefined;
  ...
  function renderRawModal() {
    ...
    copyBtn.onclick = () => {
      try {
        const snapshot = {
          __type: "TB_WIDGET_SNAPSHOT", __version: "DataHub V1.6.1",
          __copiedAt: new Date().toISOString(),
          sharedAssetObj: SHARED.DATAHUB_V16?.sharedAssetObj,
          segments: SHARED.DATAHUB_V16?.segments,
          hubMeta: SHARED.DATAHUB_V16?.hubMeta,
        };
    ...
    function syncKeys() {
      const dev = SHARED.DATAHUB_V16?.sharedAssetObj?.[selAsset.value]?.devices?.[selDev.value];
    ...
    function renderTable() {
      const assetId = selAsset.value;
      const deviceId = selDev.value;
      const keyName = selKey.value;
      const tzIana = SHARED.DATAHUB_V16?.sharedAssetObj?.[assetId]?.timezone;
      ...
      statsEl.innerHTML = `<b>${escHtml(selKey.value || "—")}</b><br/>Points: ${filtered.length}<br/>Range: ${filtered[0]?.ts ? fmtTs(filtered[0].ts, tzIana) : "—"} → ${filtered.at(-1)?.ts ? fmtTs(filtered.at(-1).ts, tzIana) : "—"}`;
      ...
      for (let i = 0; i < slice.length; i++) {
        ...
        td1.textContent = fmtTs(p.ts, tzIana);
    ...
    function syncFromHub(resetPage = true) {
      const hub = SHARED.DATAHUB_V16;
      const assetsNow = getAssetsList();
      if (!hub || !assetsNow.length) { setEmptyModalUI("No selected assets (hub is empty)"); return; }
      setActiveModalUI();

      const prevAsset = selAsset.value;
      const prevDev = selDev.value;
      const prevKey = selKey.value;
  ...
  /* ========================== LOAD ASSETS =========================== */
  const assets = await http.get(assetUrl).toPromise();

  if (assets.data.length === 1) {
    const rootDiv = document.getElementById("dh16-root");
    rootDiv.className = "dh16-root-hidden";
    stopRefreshLoop();
    selectedAssetIds.add(assets.data[0].id.id);
    ...
    async function applySelectionChange() {
      ...
      SHARED.DATAHUB_V16 = {
        ...
        hubMeta: { clearedAt: now(), revision: (SHARED.DATAHUB_V16?.hubMeta?.revision || 0) + 1, empty: true },
      };
  ...
  self.onDestroy = function () {
    ...
    try { stopRefreshLoop(); } catch (e) { console.error("RefreshTimer", e); }
    try { document.removeEventListener("click", () => {}); } catch (e) {}
    try {
      if (modalBound && modalHubListener) window.removeEventListener(HUB_EVENT, modalHubListener);
    } catch (e) {}
    try { modal.remove(); } catch (e) {}
    try { style.remove(); } catch (e) {}
    try {
      if (SHARED.DATAHUB_V16) { SHARED.DATAHUB_V16 = null; delete SHARED.DATAHUB_V16; }
    } catch (e) {}
